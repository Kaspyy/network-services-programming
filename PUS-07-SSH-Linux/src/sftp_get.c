...
int main(int argc, char **argv)
{
    ...
    scanf("%s", filename);
    strncat(dir, "/", 1);
    strncat(dir, filename, strlen(filename));
    ...
    /* Pobranie uchwytu SFTP do zdalnego pliku. */
    ...
    {
        fprintf(stdout, "Pobieranie pliku...\n");
        while (1)
        {
            /* Pobranie danych z pliku. */
            bytes = libssh2_sftp_read(handle, mem, BUF_SIZE);
            if (bytes > 0)
            {
                /* Zapis danych do pliku. */
                fwrite(mem, 1, bytes, file);
                fprintf(stdout, "Zapisywanie danych...\n");
            }
            else
            {
                /* Koniec pliku. */
    ...
    if (fclose(file) == 0)
    {
        fprintf(stdout, "Plik zostal pobrany.\n");
    }
    else
    {
